...
const CollegeAdminSharad = () => {
    ...
    const fetchStudents = async () => {
        ...
        try {
            ...
            const res = await axios.get(`http://localhost:8000/admin/students?college=Sharad%20Institute%20of%20technology&t=${timestamp}`, {
                headers: {
    ...
    return (
        <div className="animate-slide-up" style={{ marginTop: '2rem' }}>
            <header className="flex-mobile-col text-mobile-center" style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '3rem', gap: '1rem' }}>
                <div>
                    <h1 style={{ fontSize: '2.5rem', marginBottom: '0.5rem' }}>Sharad Admin Dashboard</h1>
                    <p style={{ color: 'var(--text-muted)' }}>Overview of student mental health and stress levels for Sharad Institute of Technology.</p>
                </div>
...
export default CollegeAdminSharad;
